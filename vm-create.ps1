...
$i=1
while($i -le $vm_count) {
	New-VM -VMHost 192.168.0.56 -Name nested-esxi+$i -Datastore $ds -DiskMB $diskspace -MemoryMB $memory -NumCpu $numcpu -GuestID $guestid -NetworkName $network1,$network2
	$i ++
}

# setup the cpu hardware virtualization functions 
$vm=Get-VM nested-esxi*
